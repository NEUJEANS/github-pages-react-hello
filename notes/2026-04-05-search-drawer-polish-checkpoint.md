...
## What changed
- Extracted the search drawer matching logic into a focused helper module (`src/components/search-drawer.js`) so the query behavior is easier to test and keep stable.
...
- Added a small Node test file (`src/components/search-drawer.test.js`) covering the default preview, dedupe behavior, multi-field matching, and unmatched-query empty state.
...
3. Consider narrow coverage around `space-profile.jsx` later, but only if it can stay small and avoid tooling churn.
