...
export function buildAuthSessionNotice(session) {
  ...
  return {
    ...
    body: restoredBits.length
      ? `${mergeLabel}${draftContextCopy}${handoffCopy}${transportCopy} ${restoredBits.join(' · ')} 복원 내용을 이번 세션에 반영했어요.`.trim()
      : `${mergeLabel}${draftContextCopy}${handoffCopy}${transportCopy}`.trim(),
    restoredBits,
    draftContextBits,
  }
}
